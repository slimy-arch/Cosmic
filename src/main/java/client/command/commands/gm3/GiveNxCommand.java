...
public class GiveNxCommand extends Command {
    ...
    public void execute(Client c, String[] params) {
        ...
        long value;   // NX Credit is uncapped to GameConstants.MAX_NX_CREDIT; the other two stay int
        int type = 1;
        if (params.length > 1) {
        ...
        if (victim != null) {
            if (type == CashShop.NX_CREDIT) {
                victim.getCashShop().gainNxCredit(value);
            } else {
                victim.getCashShop().gainCash(type, (int) Math.max(Integer.MIN_VALUE, Math.min(value, Integer.MAX_VALUE)));
            }
